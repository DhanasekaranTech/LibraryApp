...
const UpdateUserBookForm: React.FC<UpdateUserBookFormProps> = ({ UBID, startdate, enddate, onUpdate, onClose }) => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await api.patch(`/admin/updateUB/${UBID}`, {
        startdate: startDate,
        enddate: endDate
      }, {
        headers: {
  ...
  return (
    <div className="update-form bg-light w-25 p-4 mb-2 shadow-lg p-3 mb-2 bg-body rounded-3">
      ...
      <button className='btn btn-primary btn-sm mt-2 mb-2' onClick={handleUpdate}>Update</button>
      <button  className='btn btn-danger btn-sm mt-2 ms-2 mb-2' onClick={onClose}>Cancel</button>
    </div>
  );
};
