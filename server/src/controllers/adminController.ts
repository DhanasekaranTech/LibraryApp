...
export const showbook = async (req: Request, res: Response) => {
 ...
 export const deletebook = async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(`Received request to delete book with ID (string): ${bookIdStr}`); // Debugging log
  
      const bookId = parseInt(bookIdStr, 10);
      console.log(`Parsed book ID (number): ${bookId}`); // Debugging log
      if (isNaN(bookId)) {
        console.log(`***********Received request to delete book with ID: ${bookId}`); // Debugging log

        return res.status(400).json({ status: 400, message: 'Invalid book ID' });
      }
...
export const deleteUB = async (req: Request, res: Response) => {
    try {
      const UBIDstr = req.params.id;
      const UBID = parseInt(UBIDstr, 10)
      if (isNaN(UBID)) {
        return res
          .status(400)
