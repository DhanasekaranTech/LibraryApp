...
export const addNewBook = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ status: 201, message: "Record added successfully" });
  } catch (error) {
...
export const deletebook = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log(
      `Received request to delete book with ID (string): ${bookIdStr}`
    ); // Debugging log
    ...
    if (isNaN(bookId)) {
      console.log(
        `***********Received request to delete book with ID: ${bookId}`
      ); // Debugging log
